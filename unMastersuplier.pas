   ...
   if spcv1.Eof then
    ...
    while not spcv1.Eof do begin
        ...
        for i := 1 to Grid2.RowCount - 1 do
          ...
          Grid2.Cells[5,counter]:=spcv1.FieldByName('email_supp').AsString;
...
begin
  ...
  With Qbuatno do
    begin
      Active := false;
      SQL.Clear;                  // juml 4
      SQL.Add('Select Max(right(no_supp, 4)) from tbl_supplier');
      Active := true;
    end;
...
begin
  ...
  begin
    ...
    qv.Connection := DmModul.con1;
    qv.SQL.Clear;
    qv.SQL.Text := 'SELECT * FROM tbl_supplier WHERE no_supp = '+QuotedStr(edit1.Text)+'';
    qv.Open;
  ...
  begin
    ...
    Edit1.Enabled := false;
    tambah:=false;
    edit1.Text:=qv.FieldByName('no_supp').AsString;
    edit2.Text:=qv.FieldByName('nama_supp').AsString;
    edit3.Text:=qv.FieldByName('nama_pet').AsString;
    edit4.Text:=qv.FieldByName('nohp_pet').AsString;
    edit5.Text:=qv.FieldByName('alm_supp').AsString;
    edit6.Text:=qv.FieldByName('email_supp').AsString;
...
begin
  ...
  qv.Connection := DmModul.con1;
  qv.SQL.Clear;
  qv.SQL.Text := 'Select * from tbl_pembelian where no_supplier = '+QuotedStr(edit1.Text)+'';
  qv.Open;
    if qv.RecordCount > 0 then
      begin

        if MessageDlg('suplier ada transaksi Pembelian '+#13+#10+edit1.Text+#13+#10+
        'gagal Hapus, ?????',
        mtWarning, [mbOK, mbCancel], 0) = mrOK then
  begin
       BitBtn5.Click;
 ...
 try
   ...
   with QJenisDML do
    begin
     ...
     Close;
     SQL.Text:='DELETE FROM tbl_supplier WHERE no_supp='+''''+edit1.text+'''';
     if MessageDlg('Anda yakin akan menghapus no_supp '+edit1.text+'?',mtConfirmation,[mbYES,mbNO],0)=id_YES then
        begin
          ExecSQL;
...
procedure Tfrm_mcv.ComboBox1Change(Sender: TObject);
begin
...
begin
 ...
 try
  ...
  begin
   ...
   Close;
   if tambah then
      SQL.Text:='insert tbl_supplier (no_supp,nama_supp,nama_pet,nohp_pet,alm_supp,email_supp,tgl_create) Values('+
      ...
      SQL.Text:='UPDATE tbl_supplier SET no_supp='+''''+edit1.Text+''''+
      ',nama_supp='+''''+edit2.Text+''''+
      ',nama_pet='+''''+edit3.Text+''''+
      ',nohp_pet='+''''+edit4.Text+''''+
      ',alm_supp='+''''+edit5.Text+''''+
      ',email_supp='+''''+edit6.Text+''''+
      ',tgl_create='+QuotedStr(FormatDateTime('yyyy/mm/dd hh:nn:ss',now))+
      ' WHERE no_supp='+''''+Edit1.Text+'''';
...
procedure Tfrm_mcv.BtnHapusClick(Sender: TObject);
begin
delete;
end;
