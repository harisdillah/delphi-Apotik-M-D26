...
Procedure TFrmReturJual.KosongTex(Kunci:Boolean);
//var
//j:Integer;
Begin
  if Kunci=True then
...
Procedure TFrmReturJual.aturgrid1;
...
procedure TFrmReturJual.btcariClick(Sender: TObject);
begin
TampilDatacariTGL;
end;

procedure TFrmReturJual.btnBtnHapusClick(Sender: TObject);
begin
PnOperasi.Caption := 'Hapus Data';
PnOperasi.Color := clred;
end;

procedure TFrmReturJual.btnCariFakturClick(Sender: TObject);
begin
  FCarisp:=TFCarisp.Create(Application);
  FCarisp.Label4.Caption :='sp_GETCariPenjualan';
  LsvSetCol('No. Trans|tgl|Nilai|cara Bayar|',
  '150|100|120|160|',FCarisp.LsCari);
  FCarisp.LsField.Items.Clear;
  FCarisp.LsField.Items.Add('xxx'); //sp
  //nama colom
  FCarisp.LsField.Items.Add('no_tran_jual');
  FCarisp.LsField.Items.Add('tgl_jual');
  FCarisp.LsField.Items.Add('total_jual');
  FCarisp.LsField.Items.Add('uraian');
  FCarisp.ShowModal;
...
procedure TFrmReturJual.btnsaveClick(Sender: TObject);
var
...
begin
      if MessageDlg('Anda yakin ingin Simpan Return Jual ...?'+cxNoRetur.Text,mtConfirmation,[mbYes,mbNo],0)=mryes then
          begin
   ...
   begin
    ...
    qvr.SQL.Clear;
    qvr.SQL.Text := 'Select * from tbl_returjual where no_returjual = '+QuotedStr(cxNoRetur.Text)+'';
    qvr.Open;
  ...
  begin
      ...
      try
         sql := 'insert into tbl_returjual (no_returjual,tgl_returjual,'+
        'tgl_faktur,no_trans,tgl_create) Values('+
...
//        QuotedStr(tgljatuhtempo)+','+
        ...
        ShowMessage('Simpan baru Return jual gagal :'+ cxNoRetur.Text);
        raise;
      end;
      ...
      try
        for i1:=1 to Grid1.RowCount-1 do begin
             sql2:='INSERT INTO detail_returjual (no_trans_returjual,no,tgl_returjual,kode_brg,nama,jumlah,retur_jual,satuan)'+
             ' Values('+
             QuotedStr(cxNoRetur.Text)+','+
             ...
             QuotedStr(Grid1.Cells[4,i1])+','+  //harga
             QuotedStr(Grid1.Cells[5,i1])+','+  //Total
    ...
    // cetakstruk(btnnoreg.Text);
     KosongTex(True);
      ...
      ShowMessage('Simpan Detail gagal ok :'+ Edkodetransaksi.Text);
   ...
   begin
        ...
        try
      // ubah
      sql1 :='Update tbl_returjual set tgl_returjual='+QuotedStr(FormatDateTime('yyyy/mm/dd',dtp_TglRetur.Date))+
      ' where no_returjual='+QuotedStr(cxNoRetur.Text);
      ExecSQL1(sql1);
      ...
      //simpan Detail
      SQL := 'delete from detail_returjual where no_trans_returjual='+
        QuotedStr(cxNoRetur.Text)+'';
        ExecSQL1(sql); //Hapus
   ...
   begin
      ...
      try
      // delete
        SQL := 'delete from tbl_returjual where no_returjual='+
        QuotedStr(cxNoRetur.Text)+'';
        ExecSQL1(sql); //Hapus
...
procedure TFrmReturJual.chkgrid1Click(Sender: TObject);
begin
...
procedure TFrmReturJual.EdCariSupplierChange(Sender: TObject);
begin
if chkgrid1.checked then
...
procedure TFrmReturJual.grid1DrawCell(Sender: TObject; ACol, ARow: Integer;
...
procedure TFrmReturJual.grid1GetAlignment(Sender: TObject; ARow, ACol: Integer;
  var HAlign: TAlignment; var VAlign: TVAlignment);
begin
case ACol of
    0,1: HAlign := taCenter;
...
procedure TFrmReturJual.grid1GetCellColor(Sender: TObject; ARow, ACol: Integer;
  AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
begin
// baris Kolom 1
...
procedure TFrmReturJual.grid1GetFloatFormat(Sender: TObject; ACol,
...
case ACol of
  ...
  procedure TFrmReturJual.grid1SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
  var
  input_box : string;
begin
...
procedure TFrmReturJual.Grid2DblClickCell(Sender: TObject; ARow, ACol: Integer);
begin
    cxNoRetur.Text:=Grid2.Cells[4,AROW];
...
Procedure TFrmReturJual.TampilData_FakturBeli;
  var
    q2p,q3f : TMyQuery;
  ...
  q2p.Active := false;
  q2p.SQL.Clear;
  q2p.SQL.Text :='select * from tbl_penjualan where no_tran_jual='+QuotedStr(Edkodetransaksi.Text)+'';
  q2p.Open;
  ...
  if PnOperasi.Caption='Tambah Data' then
   begin
    ...
    dptTglFaktur.date:=q2p.FieldByName('tgl_jual').AsDateTime;
    ...
    begin
    ShowMessage( 'data kosong Penjualan / Tidak ADA : ' + Edkodetransaksi.Text );
    end;
  ...
  try
    ...
    q3f.SQL.Clear;
    q3f.SQL.Text :='SELECT * ' +
    ' FROM detail_penjualan WHERE no_penjualan LIKE'+
...
//  spdetailusul.Open;
  //Execute;
    ...
    while not q3f.Eof do begin
          ...
          Grid1.Cells[3,counter]:=dataRow(q3f.FieldByName('nm_brg_jual'),'-');
          Grid1.Cells[4,counter]:=dataRow(q3f.FieldByName('jml_jual'),'0');
          Grid1.Cells[5,counter]:='0';
          Grid1.Cells[6,counter]:=dataRow(q3f.FieldByName('sat_jual'),'-');
          //Grid1.Cells[6,counter]:=dataRow(q3f.FieldByName('jumlah'),'0');
          //Grid1.Cells[7,counter]:=dataRow(q3f.FieldByName('stok'),'-');
          q3f.Next;
    end;
   Total;
   //urut grid otomatis
...
Ntotal:=0;
   ...
   for i:=1 to Grid1.RowCount-1 do begin
        ...
        Ntotal:=Ntotal+strtofloat(Grid1.Cells[5,i]);
   end;
...
Function TFrmReturJual.GetNoBBK:string;
var nomor,faktur:string;
NO:integer;
begin
// nomor:='';
 nomor:=lastKASIR('RB'+formatdatetime('yymmdd',now())+'%');
 if nomor='' then
 faktur:='RJ'+formatdatetime('yymmdd',now())+'-0001'
 else
 begin
   ...
   faktur:='RJ'+formatdatetime('yymmdd',now())+'-'+nomor;
 end;
result:=faktur;
end;

Function TFrmReturJual.Lastkasir(s:string):string;
var QCari : TMyQuery;
...
QCari.SQL.text:='SELECT MAX(no_returjual) AS NOTA'+
     ' FROM tbl_returjual'+
     ' WHERE no_returjual LIKE '+quotedstr(s);
QCari.Open;
...
procedure TFrmReturJual.SpeedButton7Click(Sender: TObject);
begin
pnlcari.Visible:= true;
end;

procedure TFrmReturJual.SpeedButton8Click(Sender: TObject);
begin
pnlcari.Visible:= false;
end;

Procedure TFrmReturJual.aturgrid2;
//var counter,hapus_baris : Integer;
begin
  counter:=0;
  hapus_baris:=0;
...
procedure TFrmReturJual.TampilDatacariTGL;
var
  spdr : TMyStoredProc;
  i : integer;
  begin
  ...
  try
    spdr.Close;
    spdr.StoredProcName := 'sp_GETReturJualTgl';
    spdr.PrepareSQL;
    ...
    while not spdr.Eof do begin
          ...
          Grid2.Cells[1,counter]:=spdr.FieldByName('no_returjual').AsString;
          Grid2.Cells[2,counter]:=spdr.FieldByName('tgl_returjual').AsString;
          //Grid2.Cells[3,counter]:=spdr.FieldByName('nama_supp').AsString;
          Grid2.Cells[4,counter]:=spdr.FieldByName('no_returjual').AsString;
          //Grid2.Cells[5,counter]:=spdr.FieldByName('status_byr').AsString;
          Grid2.Cells[6,counter]:=spdr.FieldByName('no_trans').AsString;
...
Procedure TFrmReturJual.TampilData;
  var
    q2r,qd2r : TMyQuery;
   // i2 : Integer;
  Begin
  ...
  q2r.Active := false;
  q2r.SQL.Clear;
  q2r.SQL.Text :='select * from tbl_returjual where no_returjual='+QuotedStr(cxNoRetur.Text)+'';
  q2r.Open;
  ...
  if PnOperasi.Caption='Ubah Data' then
   begin
    ...
    dtp_TglRetur.date:=q2r.FieldByName('tgl_returjual').AsDateTime;
    cxNoRetur.Text:=q2r.FieldByName('no_returjual').AsString;
...
//    end;
  ...
  try
    qd2r.Active := false;
    qd2r.SQL.Clear;
    qd2r.SQL.Text :='select * from detail_returjual WHERE no_trans_returjual ='+
   ...
   // Total;
    ...
    while not qd2r.Eof do begin
          ...
          Grid1.Cells[5,counter]:=dataRow(qd2r.FieldByName('retur_jual'),'0');
