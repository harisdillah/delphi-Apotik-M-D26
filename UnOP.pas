...
procedure TFrmOp.AdvcboDrawProp(Sender: TObject; RowIndex,
...
procedure TFrmOp.deleterow(mg:TStringGrid;ARow:integer);
var i,j,k:integer;
begin
        with mg do begin
...
Grid1.Cells[4,Grid1.RowCount-1]:=edjml.Text;//jml
Grid1.Cells[5,Grid1.RowCount-1]:=EdNilai.Text;//nilai
Grid1.Cells[6,Grid1.RowCount-1]:=edtotal.Text;//Total
...
Procedure TFrmOp.jumlah_permintaan;
...
procedure TFrmOp.BitBtn1Click(Sender: TObject);
begin
TampilDataNocari;
end;

Procedure TFrmOp.aturgrid1;
//var counter,hapus_baris : Integer;
begin
  counter:=0;
  hapus_baris:=0;
...
Procedure TFrmOp.KosongTex(Kunci:Boolean);
//var
//j:Integer;
Begin
  if Kunci=True then
...
try
   ...
   q_no.Active := false;
   q_no.SQL.Clear;
   q_no.SQL.Add('Select Max(right(no_trans_op, 4)) from h_op ' +
              'Where Mid(no_trans_op, 3,6) = ' + QuotedStr(tgl) + ';'); //JUMLAH NOMOR
   q_no.Active := true;
...
procedure TFrmOp.SpeedButton1Click(Sender: TObject);
begin
//if frm_mcv=nil then
...
procedure TFrmOp.SpeedButton2Click(Sender: TObject);
begin
btnnoreg.Text:=GetNoBBK;
end;

procedure TFrmOp.btnBtnBaruClick(Sender: TObject);
begin
PnOperasi.Caption := 'Tambah Data';
btnnoreg.Text:=GetNoBBK;
btnnoreg.Enabled:=False;
...
procedure TFrmOp.cxButtonEdit1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
// end;
  ...
  Procedure TFrmOp.Tampil_combo_Petugas;
 var
...
procedure TFrmOp.btnBtnUbah1Click(Sender: TObject);
begin
...
procedure TFrmOp.btnBtnHapusClick(Sender: TObject);
begin
PnOperasi.Caption := 'Hapus Data';
end;

procedure TFrmOp.btcariClick(Sender: TObject);
begin
TampilDatacari;
end;

procedure TFrmOp.btn1Click(Sender: TObject);
begin
tambah_grid;
end;

procedure TFrmOp.cxcariKeyPress(Sender: TObject; var Key: Char);
begin
if(Key=#13)then
	begin
    try
...
function TFrmOp.Caribrg(nkode_cr: String):string;
var
  q6 : TMyStoredProc;
  dt:TListItem;
  //nSem:string;
...
procedure TFrmOp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FrmOp:=nil;
  Action:=caFree;
end;

procedure TFrmOp.lv1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmOp.lv1KeyPress(Sender: TObject; var Key: Char);
begin
//try
//		if(Key=#13)then
//	begin
...
procedure TFrmOp.edjmlKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFrmOp.edjmlChange(Sender: TObject);
begin
HitungTotal;
end;

procedure TFrmOp.edjmlKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      {
...
procedure TFrmOp.Total;
begin
JML_BRG:=0;
total_nilai:=0;
   for i:=1 to Grid1.RowCount-1 do begin
        JML_BRG:=JML_BRG+strtofloat(Grid1.Cells[4,i]);
        total_nilai:=total_nilai+strtofloat(Grid1.Cells[6,i]);
   end;
   edt_jml_barang.Text:=floattostr(JML_BRG);
   ed_t1.Text:=floattostr(total_nilai);
   //formatRp(ed_t1);
end;

procedure TFrmOp.HitungTotal;
begin
//qty,jml,harga,total_harga,disk,total2 : real ; // :integer;
if edjml.text = '' then Exit;
if EdNilai.text = '' then Exit;
//formatRupiah(edt2);
jml :=strtofloat (clearDot(edjml.Text)); //Jumlah
harga :=strtofloat (clearDot(EdNilai.Text)); //Harga

total_harga := jml * harga;  //T2
edtotal.Color:=clcream;
edtotal.Text := floattostr(total_harga);

end;


procedure TFrmOp.ed2PropertiesChange(Sender: TObject);
begin
...
procedure TFrmOp.btn2Click(Sender: TObject);
begin
 if (cek_baris='') then begin
...
procedure TFrmOp.grid1SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
  var
    input_box : string;
begin
...
if (aCol = 4) then
   ...
   //Uraian
    ...
    if not (Grid1.RowCount=1) then begin
        //input_box:=InputBox('No. Uraian ','Uraian : '+Grid1.Cells[3,arow],Grid1.Cells[3,arow]);
          FCarisp:=TFCarisp.Create(Application);
  FCarisp.Label4.Caption :='sprekening';
  LsvSetCol('no.rek|uraian|',
  '120|270|',FCarisp.LsCari);
  FCarisp.LsField.Items.Clear;
  FCarisp.LsField.Items.Add('xxx'); //sp
  //nama colom
  FCarisp.LsField.Items.Add('no_rek');
  FCarisp.LsField.Items.Add('ket');
  FCarisp.ShowModal;

  if nFieldCari[0]<>'' then
  begin
   Grid1.Cells[ACol,ARow]:=nFieldCari[0];
 //   cx_nama_brg.Text:=nFieldCari[1];
 //   btn_ruangan.Style.Color:=clMoneyGreen;
   ...
   //jumlah brg
    ...
    if not (Grid1.RowCount=1) then begin
       input_box:=InputBox('qty','Nilai  : '+Grid1.Cells[3,arow]+' -> '+Grid1.Cells[5,arow],Grid1.Cells[5,arow]);
       qty:=StrToFloat(input_box);
       Grid1.Cells[ACol,ARow]:=FloatToStr(qty);
          jml:=StrToFloat(Grid1.Cells[4,arow]);
          harga:=StrToFloat(Grid1.Cells[5,arow]);
...
procedure TFrmOp.Grid2DblClickCell(Sender: TObject; ARow, ACol: Integer);
begin
...
procedure TFrmOp.Grid2GetAlignment(Sender: TObject; ARow, ACol: Integer;
  var HAlign: TAlignment; var VAlign: TVAlignment);
begin
case ACol of
    1: HAlign := taCenter;
...
procedure TFrmOp.Grid2GetFloatFormat(Sender: TObject; ACol, ARow: Integer;
  var IsFloat: Boolean; var FloatFormat: string);
begin
isFloat := Acol in [4];
case ACol of
...
procedure TFrmOp.grid1GetAlignment(Sender: TObject; ARow,
  ACol: Integer; var HAlign: TAlignment; var VAlign: TVAlignment);
begin
case ACol of
    1: HAlign := taCenter;
    2,3: HAlign := taLeftJustify;
    4,5,6: HAlign := taRightJustify;
  end;
end;

procedure TFrmOp.grid1GetFloatFormat(Sender: TObject; ACol,
  ARow: Integer; var IsFloat: Boolean; var FloatFormat: String);
begin
isFloat := Acol in [4,5,6];
case ACol of
  4: Floatformat :='%.0n';
  5: Floatformat :='%.0n';
  6: Floatformat :='%.0n';
...
procedure TFrmOp.ed_dtotalChange(Sender: TObject);
begin
   //formatRupiah(ed_dtotal);
   //edtotal.Text := ed_dtotal.Text;
end;

procedure TFrmOp.ed_t1Change(Sender: TObject);
begin
...
procedure TFrmOp.grid1GetCellColor(Sender: TObject; ARow,
...
procedure TFrmOp.tmr1Timer(Sender: TObject);
begin
...
procedure TFrmOp.edjmlPropertiesChange(Sender: TObject);
begin
{
if edjml.Text = '' then Exit;
...
procedure TFrmOp.edkodePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  FCarisp:=TFCarisp.Create(Application);
  FCarisp.Label4.Caption :='sprekening';
...
procedure TFrmOp.EdNilaiChange(Sender: TObject);
begin
HitungTotal;
end;

procedure TFrmOp.cxBtn_idcvChange(Sender: TObject);
var
...
procedure TFrmOp.cxBtn_idcvClickBtn(Sender: TObject);
begin
     FCarisp:=TFCarisp.Create(Application);
...
procedure TFrmOp.chk1Click(Sender: TObject);
begin
      if chk1.Checked   THEN
      BEGIN
      ed2.Visible := True;
...
procedure TFrmOp.btnsaveClick(Sender: TObject);
var
// simpan cbb
...
begin
   ...
   begin
    ...
    qv.SQL.Clear;
    qv.SQL.Text := 'Select * from h_op where no_trans_op = '+QuotedStr(btnnoreg.Text)+'';
    qv.Open;
  ...
  begin
      ...
      try
         sql := 'insert into h_op (no_trans_op,tgl_op,tgl_create,ket,nilai,'+
        'id_peg) Values('+
...
//        ExecSQL1(sql3);
      ...
      try
        for i1:=1 to Grid1.RowCount-1 do begin
             sql2:='INSERT INTO detail_op (no_op,tgl_op,no,kode_rek,nama,jumlah,nilai,total)'+
             ' Values('+
             QuotedStr(btnnoreg.Text)+','+  //no trans
             QuotedStr(FormatDateTime('yyyy/mm/dd',dtp_buat.Date))+','+ //tgl
             QuotedStr(Grid1.Cells[1,i1])+','+ // no
             QuotedStr(Grid1.Cells[2,i1])+','+ // kode rek
             QuotedStr(Grid1.Cells[3,i1])+','+ // nama
             QuotedStr(RealFormat(StrToFloat(Grid1.Cells[4,i1])))+','+// jumlah 0,00
             QuotedStr(Grid1.Cells[5,i1])+','+ // nilai
             QuotedStr(Grid1.Cells[6,i1])+')'; //
             ExecSQL1(sql2);
             end;
   ...
   begin
        ...
        try
      // ubah
      sql1 :='Update h_op set tgl_op='+QuotedStr(FormatDateTime('yyyy/mm/dd',dtp_buat.Date))+
      ',ket='+QuotedStr(cxNoBbk.Text)+
      ',id_peg='+QuotedStr(cbopetugas)+
      ',nilai='+clearDot(ed_t1.Text)+
      ' where no_trans_op='+QuotedStr(btnnoreg.Text);
      ExecSQL1(sql1);
      ...
      //simpan Detail
       sql:= 'delete from detail_op'+
        ' where no_op='+QuotedStr(btnnoreg.Text);
        ExecSQL1(sql);
   ...
   begin
      ...
      try
      // delete
        SQL := 'delete from h_op where no_trans_op='+
        QuotedStr(btnnoreg.Text)+'';
        ExecSQL1(sql); //Hapus
...
Procedure TFrmOp.TampilDataNocari;
  var
  ...
  Procedure TFrmOp.TampilDatacari;
  var
  spdata : TMyStoredProc;
  i : integer;
  begin
  ...
  try
    spdata.Close;
    spdata.StoredProcName := 'sp_GETOP';
    spdata.PrepareSQL;
    spdata.ParamByName('dtgl1').AsString := FormatDateTime('yyyy-mm-dd',dtp1.date); //key ba
    spdata.ParamByName('dtgl2').AsString  := FormatDateTime('yyyy-mm-dd',dtp2.date); //key ba
    //spdata.ParamByName('cari2').AsString := FMenu.dxStatusBar1.Panels[2].Text;
    ...
    while not spdata.Eof do begin
          Inc(counter);   //IntToStr(counter);
          Grid2.Cells[0,counter]:=IntToStr(counter); //No.
          Grid2.Cells[1,counter]:=spdata.FieldByName('no_trans_op').AsString;
          Grid2.Cells[2,counter]:=spdata.FieldByName('ket').AsString;
          Grid2.Cells[3,counter]:=spdata.FieldByName('tgl_op').AsString;
          Grid2.Cells[4,counter]:=spdata.FieldByName('nilai').AsString;
          Grid2.Cells[5,counter]:=spdata.FieldByName('nama_peg').AsString;
          //Grid2.Cells[5,counter]:=spdata.FieldByName('gudang').AsString;
          {
          grid2.ShowSelection := True;
          for i := 1 to grid2.RowCount - 1 do
  ...
  q2.Active := false;
  q2.SQL.Clear;
  q2.SQL.Text :='select * from h_op where no_trans_op='+QuotedStr(btnnoreg.Text)+'';
  q2.Open;
  ...
  if PnOperasi.Caption='Ubah Data' then
   begin
    ...
    KosongTex(True);
    //ambilRuang;
    btnnoreg.Text:=q2.FieldByName('no_trans_op').AsString;
    dtp_buat.date:=q2.FieldByName('tgl_op').AsDateTime;
    cxNoBbk.Text:=q2.FieldByName('ket').AsString;
    cbbpetugas.EditValue:=q2.FieldByName('id_peg').AsString;

  end
  else
    begin
    ShowMessage( 'data kosong Operasional / Tidak ADA : ' + btnnoreg.Text );
    end;
  ...
  try
    qd2.Active := false;
    qd2.SQL.Clear;
    qd2.SQL.Text :='select * from detail_op WHERE no_op ='+
    ...
    while not qd2.Eof do begin
          ...
          Grid1.Cells[2,counter]:=dataRow(qd2.FieldByName('kode_rek'),'-');
          Grid1.Cells[3,counter]:=dataRow(qd2.FieldByName('nama'),'-');
          Grid1.Cells[4,counter]:=dataRow(qd2.FieldByName('jumlah'),'0');
          Grid1.Cells[5,counter]:=dataRow(qd2.FieldByName('nilai'),'0');
          Grid1.Cells[6,counter]:=dataRow(qd2.FieldByName('total'),'0');
          qd2.Next;
    end;
   Total;
...
Function TFrmOp.GetNoBBK:string;
var nomor,faktur:string;
NO:integer;
begin
// nomor:='';
 nomor:=lastKASIR('OP'+formatdatetime('yymmdd',now())+'%');
 if nomor='' then
 faktur:='OP'+formatdatetime('yymmdd',now())+'-0001'
 else
 begin
   ...
   faktur:='OP'+formatdatetime('yymmdd',now())+'-'+nomor;
 end;
result:=faktur;
end;

Function TFrmOp.Lastkasir(s:string):string;
begin
QCari.Close;
QCari.Connection:=DmModul.con1;
QCari.SQL.text:='SELECT MAX(no_trans_op) AS NOTA'+
     ' FROM h_op'+
     ' WHERE no_trans_op LIKE '+quotedstr(s);
QCari.Open;
...
procedure TFrmOp.btn_penerimaPropertiesChange(Sender: TObject);
var
...
procedure TFrmOp.btn_ruanganPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  FCarisp:=TFCarisp.Create(Application);
...
procedure TFrmOp.btn_ruanganPropertiesChange(Sender: TObject);
begin
ambilRuang;
end;

procedure TFrmOp.lv1CustomDrawItem(Sender: TCustomListView;
...
procedure TFrmOp.grid1DrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
  //var   S: string;
begin
...
procedure TFrmOp.btnnoregPropertiesChange(Sender: TObject);
begin
Edkodetransaksi.Text := btnnoreg.Text;
end;

procedure TFrmOp.SpeedButton3Click(Sender: TObject);
begin
// uRUT
   for i2:=1 to Grid1.RowCount-1 do begin
    Grid1.Cells[0,i2]:=IntToStr(i2);
...
procedure TFrmOp.SpeedButton4Click(Sender: TObject);
begin
//PnOperasi.Caption := 'Tambah Data';
////btnnoreg.Text:=GetNoBBK;
//btnnoreg.Enabled:=False;
