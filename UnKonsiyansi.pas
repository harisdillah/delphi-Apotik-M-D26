...
Procedure TFrmKonsiyansi.ambilRuang;
var
...
procedure TFrmKonsiyansi.deleterow(mg:TStringGrid;ARow:integer);
var i,j,k:integer;
begin
        with mg do begin
...
procedure TFrmKonsiyansi.FormCreate(Sender: TObject);
begin
  KosongTex(True);
...
Procedure TFrmKonsiyansi.tambah_grid;
var
    addSat : TMyQuery;
    tglexp:string;
begin
...
Procedure TFrmKonsiyansi.cbo_satuan;
var
    qcom1 : TMyQuery;
begin
    try
...
procedure TFrmKonsiyansi.cbo_statusChange(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.cbo_statusPropertiesChange(Sender: TObject);
begin
if cbo_status.Text = 'KREDIT'  then
begin
...
procedure TFrmKonsiyansi.CheckBox1Click(Sender: TObject);
begin
...
Procedure TFrmKonsiyansi.jumlah_permintaan;
...
Procedure TFrmKonsiyansi.aturgrid2;
...
procedure TFrmKonsiyansi.BitBtn1Click(Sender: TObject);
begin
TampilDataNocari;
end;

Procedure TFrmKonsiyansi.aturgrid1;
//var counter,hapus_baris : Integer;
begin
  counter:=0;
  hapus_baris:=0;
...
Procedure TFrmKonsiyansi.KosongTex(Kunci:Boolean);
//var
//j:Integer;
Begin
  if Kunci=True then
...
Procedure TFrmKonsiyansi.buatno;
var
  tgl,urut : string;
  i_no : integer;
  q_no: TMyQuery;
...
try
   ...
   q_no.Active := false;
   q_no.SQL.Clear;
   q_no.SQL.Add('Select Max(right(no_tranxkonsi, 4)) from tbl_konsinasi ' +
              'Where Mid(no_tranxkonsi, 3,6) = ' + QuotedStr(tgl) + ';'); //JUMLAH NOMOR
   q_no.Active := true;
...
procedure TFrmKonsiyansi.SpeedButton2Click(Sender: TObject);
begin
btnnoreg.Text:=GetNoBBK;
end;

procedure TFrmKonsiyansi.btnBtnBaruClick(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.cxBtn_idcvPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TFrmKonsiyansi.cxBtn_idcvPropertiesChange(Sender: TObject);
var
...
procedure TFrmKonsiyansi.cxButtonEdit1PropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
//  FCarisp:=TFCarisp.Create(Application);
...
// end;
  ...
  Procedure TFrmKonsiyansi.Tampil_combo_Petugas;
 var
...
procedure TFrmKonsiyansi.FormShow(Sender: TObject);
...
procedure TFrmKonsiyansi.btnBtnUbah1Click(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.btnBtnHapusClick(Sender: TObject);
begin
PnOperasi.Caption := 'Hapus Data';
end;

procedure TFrmKonsiyansi.btcariClick(Sender: TObject);
begin
TampilDatacariTGL;
end;

procedure TFrmKonsiyansi.btn1Click(Sender: TObject);
begin
tambah_grid;
end;

procedure TFrmKonsiyansi.cxcariKeyPress(Sender: TObject; var Key: Char);
begin
if(Key=#13)then
	begin
    try
...
function TFrmKonsiyansi.Caribrg(nkode_cr: String):string;
var
  q6 : TMyStoredProc;
  dt:TListItem;
  //nSem:string;
...
procedure TFrmKonsiyansi.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FrmKonsiyansi:=nil;
  Action:=caFree;
end;

procedure TFrmKonsiyansi.lv1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmKonsiyansi.lv1KeyPress(Sender: TObject; var Key: Char);
begin
try
		if(Key=#13)then
	begin
...
procedure TFrmKonsiyansi.edjmlKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TFrmKonsiyansi.edjmlChange(Sender: TObject);
begin
if edjml.text = '' then Exit;
hitung;
end;

procedure TFrmKonsiyansi.edjmlKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
      {
...
procedure TFrmKonsiyansi.Total;
var
...
procedure TFrmKonsiyansi.ed2PropertiesChange(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.EdDiskPerChange(Sender: TObject);
begin
if EdDiskPer.text = '' then Exit;
hitung;
end;

procedure TFrmKonsiyansi.edhargasatChange(Sender: TObject);
begin
   if edhargasat.text = '' then Exit;
hitung;
end;

procedure TFrmKonsiyansi.hapus_grid;
begin
...
procedure TFrmKonsiyansi.btn2Click(Sender: TObject);
begin
 if (cek_baris='') then begin
...
procedure TFrmKonsiyansi.grid1SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
  var
    input_box : string;
    qty,jml,harga,total_harga,disk,total2 : real ; // :integer;
...
//   //no bacth
    ...
    if not (Grid1.RowCount=1) then begin
        input_box:=InputBox('No. Bacth ','no batch : '+Grid1.Cells[3,arow],Grid1.Cells[5,arow]);
                Grid1.Cells[ACol,ARow]:=input_box;
      end;
      end;
...
procedure TFrmKonsiyansi.Grid2DblClickCell(Sender: TObject; ARow, ACol: Integer);
begin
...
procedure TFrmKonsiyansi.Grid2GetFloatFormat(Sender: TObject; ACol,
...
procedure TFrmKonsiyansi.grid1GetAlignment(Sender: TObject; ARow,
...
procedure TFrmKonsiyansi.grid1GetFloatFormat(Sender: TObject; ACol,
  ARow: Integer; var IsFloat: Boolean; var FloatFormat: String);
begin
...
procedure TFrmKonsiyansi.ed_dtotalChange(Sender: TObject);
begin
   //formatRupiah(ed_dtotal);
   //edtotal.Text := ed_dtotal.Text;
end;

procedure TFrmKonsiyansi.ed_t1Change(Sender: TObject);
var
...
procedure TFrmKonsiyansi.grid1GetCellColor(Sender: TObject; ARow,
...
procedure TFrmKonsiyansi.tmr1Timer(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.edjmlPropertiesChange(Sender: TObject);
begin
{
if edjml.Text = '' then Exit;
...
procedure TFrmKonsiyansi.EdNpajakChange(Sender: TObject);
begin
hitungTotal;
end;

procedure TFrmKonsiyansi.EdpajakperChange(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.edt2Change(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.edTotalDiskChange(Sender: TObject);
begin
if edTotalDisk.text = '' then Exit;
hitung;
end;

procedure TFrmKonsiyansi.edTotalDiskonChange(Sender: TObject);
begin
...
procedure TFrmKonsiyansi.chk1Click(Sender: TObject);
begin
      if chk1.Checked   THEN
      BEGIN
      ed2.Visible := True;
...
procedure TFrmKonsiyansi.chkDiskonClick(Sender: TObject);
begin
if  chkDiskon.Checked then
      begin
      chkDiskon.Color := clgreen;
...
procedure TFrmKonsiyansi.btnsaveClick(Sender: TObject);
var
// simpan cbb
...
//        end;
   ...
   if (Trim(cxNoBbk.Text)='')  then
        begin
        ShowMessage('Nomor Kosinyasi Kosong..??');
        cxNoBbk.SetFocus;
        Exit;
        end;
...
begin
   ...
   begin
    ...
    qv.SQL.Clear;
    qv.SQL.Text := 'Select * from tbl_konsinasi where no_tranxkonsi = '+QuotedStr(btnnoreg.Text)+'';
    qv.Open;
    if qv.RecordCount > 0 then
      begin

        if MessageDlg('Kode Transkonsi Sudah ada'+#13+#10+btnnoreg.Text+#13+#10+
  ...
  begin
      ...
      try
         sql := 'insert into tbl_konsinasi (no_tranxkonsi,incl_disk,tgl_konsinasi,no_konsi,'+
        'status_byr,jth_tempo,no_supplier,tgl_create,total1,diskon,total2,pajakper,pajak,total_konsinasi,id_peg) Values('+
...
//
      ...
      try
        ...
        for i1:=1 to Grid1.RowCount-1 do begin
             sql2:='INSERT INTO detail_konsinasi (no_transkonsi,tgl_konsinasi,no,kode_brg,exp_obat,nobatch,nama,jumlah,satuan,hrga_konsinasi,n_t1,n_disk,n_t2)'+
             ' Values('+
   ...
   begin
        ...
        sql1 :='Update tbl_konsinasi set ' +
        'tgl_konsinasi='+QuotedStr(FormatDateTime('yyyy/mm/dd',dtp_buat.Date))+
        ',no_konsi='+QuotedStr(cxNoBbk.Text)+
        ...
        ',total_konsinasi='+QuotedStr(clearDot(edGrandTotal.Text))+
        ',id_peg='+QuotedStr(cbopetugas)+
        ' where no_tranxkonsi='+QuotedStr(btnnoreg.Text);
        ExecSQL1(sql1);
      ...
      //simpan Detail
       sql:= 'delete from detail_konsinasi'+
        ' where no_transkonsi='+QuotedStr(btnnoreg.Text);
        ExecSQL1(sql);
   ...
   begin
      ...
      try
      // delete
        SQL := 'delete from tbl_konsinasi where no_tranxkonsi='+
        QuotedStr(btnnoreg.Text)+'';
        ExecSQL1(sql); //Hapus
        ...
        //AktifControl(Self,PnInput,False);
             ...
             ShowMessage('Hapus no_transkonsi Gagal :'+ btnnoreg.Text);
        raise;
      end;
        end;
    end; //selesai Proses
...
Procedure TFrmKonsiyansi.TampilDataNocari;
  var
  ...
  Procedure TFrmKonsiyansi.TampilDatacariTGL;
  var
  spdata : TMyStoredProc;
  i : integer;
  begin
  ...
  try
    spdata.Close;
    spdata.StoredProcName := 'sp_GETKonsinyasiTgl';
    spdata.PrepareSQL;
    spdata.ParamByName('dtgl1').AsString := FormatDateTime('yyyy-mm-dd',dtp1.date); //key ba
    spdata.ParamByName('dtgl2').AsString  := FormatDateTime('yyyy-mm-dd',dtp2.date); //key ba
    //spdata.ParamByName('cari2').AsString := FMenu.dxStatusBar1.Panels[2].Text;
    ...
    while not spdata.Eof do begin
          Inc(counter);   //IntToStr(counter);
          Grid2.Cells[0,counter]:=IntToStr(counter); //No.
          Grid2.Cells[1,counter]:=spdata.FieldByName('no_konsi').AsString;
          Grid2.Cells[2,counter]:=spdata.FieldByName('tgl_konsinasi').AsString;
          Grid2.Cells[3,counter]:=spdata.FieldByName('nama_supp').AsString;
          Grid2.Cells[4,counter]:=spdata.FieldByName('total_konsinasi').AsString;
          Grid2.Cells[5,counter]:=spdata.FieldByName('status_byr').AsString;
          Grid2.Cells[6,counter]:=spdata.FieldByName('no_tranxkonsi').AsString;
  ...
  Procedure TFrmKonsiyansi.TampilData;
  var
    q2,q3 : TMyQuery;
    qd2 : TMyQuery;
    i2 : Integer;
  ...
  q2.Active := false;
  q2.SQL.Clear;
  q2.SQL.Text :='select * from tbl_konsinasi where no_tranxkonsi='+QuotedStr(btnnoreg.Text)+'';
  q2.Open;
  ...
  if PnOperasi.Caption='Ubah Data' then
   begin
    ...
    KosongTex(True);
    //ambilRuang;
    btnnoreg.Text:=q2.FieldByName('no_tranxkonsi').AsString;
    dtp_buat.date:=q2.FieldByName('tgl_konsinasi').AsDateTime;
    cxNoBbk.Text:=q2.FieldByName('no_konsi').AsString;
   ...
   //  cbo_gudang.EditValue:=q2.FieldByName('gudang').AsString;
    ...
    begin
    ShowMessage( 'data kosong no_konsinyasi / Tidak ADA : ' + btnnoreg.Text );
    end;
  ...
  try
    qd2.Active := false;
    qd2.SQL.Clear;
    qd2.SQL.Text :='select * from detail_konsinasi WHERE no_transkonsi ='+
    ...
    while not qd2.Eof do begin
      ...
      //    Grid1.Cells[4,counter]:=FormatDateTime('yyyy-mm-dd',qd2.FieldByName('exp_obat').AsDateTime);
          ...
          Grid1.Cells[8,counter]:=dataRow(qd2.FieldByName('hrga_konsinasi'),'0');
...
Function TFrmKonsiyansi.GetNoBBK:string;
var nomor,faktur:string;
NO:integer;
begin
// nomor:='';
 nomor:=lastKASIR('KO'+formatdatetime('yymmdd',now())+'%');
 if nomor='' then
 faktur:='KO'+formatdatetime('yymmdd',now())+'-0001'
 else
 begin
   ...
   faktur:='KO'+formatdatetime('yymmdd',now())+'-'+nomor;
 end;
result:=faktur;
end;

Function TFrmKonsiyansi.Lastkasir(s:string):string;
begin
QCari.Close;
QCari.Connection:=DmModul.con1;
QCari.SQL.text:='SELECT MAX(no_tranxkonsi) AS NOTA'+
     ' FROM tbl_konsinasi'+
     ' WHERE no_tranxkonsi LIKE '+quotedstr(s);
QCari.Open;
...
procedure TFrmKonsiyansi.btn_penerimaPropertiesChange(Sender: TObject);
var
...
procedure TFrmKonsiyansi.btn_ruanganPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  FCarisp:=TFCarisp.Create(Application);
...
procedure TFrmKonsiyansi.btn_ruanganPropertiesChange(Sender: TObject);
begin
ambilRuang;
end;

procedure TFrmKonsiyansi.lv1CustomDrawItem(Sender: TCustomListView;
...
procedure TFrmKonsiyansi.grid1DrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
  //var   S: string;
begin
...
procedure TFrmKonsiyansi.btnnoregPropertiesChange(Sender: TObject);
begin
Edkodetransaksi.Text := btnnoreg.Text;
end;

procedure TFrmKonsiyansi.SpeedButton3Click(Sender: TObject);
begin
// uRUT
   for i2:=1 to Grid1.RowCount-1 do begin
    Grid1.Cells[0,i2]:=IntToStr(i2);
...
procedure TFrmKonsiyansi.hitung;
begin
...
procedure TFrmKonsiyansi.hitungTotal;
var
...
procedure TFrmKonsiyansi.SpeedButton4Click(Sender: TObject);
begin
hitung;

end;

procedure TFrmKonsiyansi.SpeedButton7Click(Sender: TObject);
begin
pnlcari.Visible:= true;
end;

procedure TFrmKonsiyansi.SpeedButton8Click(Sender: TObject);
begin
pnlcari.Visible:= false;
end;
