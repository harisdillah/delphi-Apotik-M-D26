...
Procedure TFrmReturBeli.KosongTex(Kunci:Boolean);
//var
//j:Integer;
Begin
  if Kunci=True then
...
Procedure TFrmReturBeli.aturgrid1;
...
procedure TFrmReturBeli.btcariClick(Sender: TObject);
begin
TampilDatacariTGL;
end;

procedure TFrmReturBeli.btnBtnHapusClick(Sender: TObject);
begin
PnOperasi.Caption := 'Hapus Data';
PnOperasi.Color := clred;
end;

procedure TFrmReturBeli.btnCariFakturClick(Sender: TObject);
begin
  FCarisp:=TFCarisp.Create(Application);
  FCarisp.Label4.Caption :='sp_GETCariPembelian';
  ...
  //nama colom
  FCarisp.LsField.Items.Add('no_tranxp');
  FCarisp.LsField.Items.Add('tgl_pembelian');
  FCarisp.LsField.Items.Add('no_faktur');
  FCarisp.LsField.Items.Add('nama_supp');
  FCarisp.LsField.Items.Add('total_pembelian');
...
begin
   ...
   begin
    ...
    qvr.SQL.Clear;
    qvr.SQL.Text := 'Select * from tbl_returbeli where no_returbeli = '+QuotedStr(cxNoRetur.Text)+'';
    qvr.Open;
  ...
  begin
      ...
      try
         sql := 'insert into tbl_returbeli (no_returbeli,tgl_returbeli,'+
        'id_cv,nm_cv,tgl_faktur,no_faktur,no_trans,tgl_create) Values('+
...
//        QuotedStr(tgljatuhtempo)+','+
      ...
      try
        for i1:=1 to Grid1.RowCount-1 do begin
             sql2:='INSERT INTO detail_returbeli (no_trans_returbeli,no,tgl_returbeli,kode_brg,nama,jumlah,retur,satuan)'+
             ' Values('+
             QuotedStr(cxNoRetur.Text)+','+
   ...
   begin
        ...
        try
      // ubah
      sql1 :='Update tbl_returbeli set tgl_returbeli='+QuotedStr(FormatDateTime('yyyy/mm/dd',dtp_TglRetur.Date))+
      ' where no_returbeli='+QuotedStr(cxNoRetur.Text);
      ExecSQL1(sql1);
      ...
      //simpan Detail
      SQL := 'delete from detail_returbeli where no_trans_returbeli='+
        QuotedStr(cxNoRetur.Text)+'';
        ExecSQL1(sql); //Hapus
   ...
   begin
      ...
      try
      // delete
        SQL := 'delete from tbl_returbeli where no_returbeli='+
        QuotedStr(cxNoRetur.Text)+'';
        ExecSQL1(sql); //Hapus
...
procedure TFrmReturBeli.chkgrid1Click(Sender: TObject);
begin
...
procedure TFrmReturBeli.EdCariSupplierChange(Sender: TObject);
begin
if chkgrid1.checked then
...
procedure TFrmReturBeli.grid1DrawCell(Sender: TObject; ACol, ARow: Integer;
...
procedure TFrmReturBeli.grid1GetAlignment(Sender: TObject; ARow, ACol: Integer;
  var HAlign: TAlignment; var VAlign: TVAlignment);
begin
case ACol of
    0,1: HAlign := taCenter;
...
procedure TFrmReturBeli.grid1GetCellColor(Sender: TObject; ARow, ACol: Integer;
  AState: TGridDrawState; ABrush: TBrush; AFont: TFont);
begin
// baris Kolom 1
...
procedure TFrmReturBeli.grid1GetFloatFormat(Sender: TObject; ACol,
...
case ACol of
  ...
  procedure TFrmReturBeli.grid1SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
  var
  input_box : string;
begin
...
procedure TFrmReturBeli.Grid2DblClickCell(Sender: TObject; ARow, ACol: Integer);
begin
...
Procedure TFrmReturBeli.TampilData_FakturBeli;
  var
    q2p,q3f : TMyQuery;
  ...
  q2p.Active := false;
  q2p.SQL.Clear;
  q2p.SQL.Text :='select * from tbl_pembelian where no_tranxp='+QuotedStr(Edkodetransaksi.Text)+'';
  q2p.Open;
  ...
  if PnOperasi.Caption='Tambah Data' then
   begin
    ...
    dptTglFaktur.date:=q2p.FieldByName('tgl_pembelian').AsDateTime;
  ...
  try
    ...
    q3f.SQL.Clear;
    q3f.SQL.Text :='SELECT * ' +
    ' FROM detail_pembelian WHERE no_trans LIKE'+
...
Function TFrmReturBeli.GetNoBBK:string;
...
// nomor:='';
 ...
 if nomor='' then
 faktur:='RB'+formatdatetime('yymmdd',now())+'-0001'
 else
 begin
   ...
   faktur:='RB'+formatdatetime('yymmdd',now())+'-'+nomor;
 end;
result:=faktur;
end;

Function TFrmReturBeli.Lastkasir(s:string):string;
var QCari : TMyQuery;
...
QCari.SQL.text:='SELECT MAX(no_returbeli) AS NOTA'+
     ' FROM tbl_returbeli'+
     ' WHERE no_returbeli LIKE '+quotedstr(s);
QCari.Open;
...
procedure TFrmReturBeli.SpeedButton7Click(Sender: TObject);
begin
pnlcari.Visible:= true;
end;

procedure TFrmReturBeli.SpeedButton8Click(Sender: TObject);
begin
pnlcari.Visible:= false;
end;

Procedure TFrmReturBeli.aturgrid2;
//var counter,hapus_baris : Integer;
begin
  counter:=0;
  hapus_baris:=0;
...
procedure TFrmReturBeli.TampilDatacariTGL;
var
  spdr : TMyStoredProc;
  i : integer;
  begin
  ...
  try
    spdr.Close;
    spdr.StoredProcName := 'sp_GETReturBeliTgl';
    spdr.PrepareSQL;
    ...
    while not spdr.Eof do begin
          ...
          Grid2.Cells[2,counter]:=spdr.FieldByName('tgl_returbeli').AsString;
          Grid2.Cells[3,counter]:=spdr.FieldByName('nama_supp').AsString;
          Grid2.Cells[4,counter]:=spdr.FieldByName('no_returbeli').AsString;
          //Grid2.Cells[5,counter]:=spdr.FieldByName('status_byr').AsString;
          Grid2.Cells[6,counter]:=spdr.FieldByName('no_returbeli').AsString;
...
Procedure TFrmReturBeli.TampilData;
  var
    q2r,qd2r : TMyQuery;
   // i2 : Integer;
  Begin
  ...
  q2r.Active := false;
  q2r.SQL.Clear;
  q2r.SQL.Text :='select * from tbl_returbeli where no_returbeli='+QuotedStr(cxNoRetur.Text)+'';
  q2r.Open;
  ...
  if PnOperasi.Caption='Ubah Data' then
   begin
    ...
    dtp_TglRetur.date:=q2r.FieldByName('tgl_returbeli').AsDateTime;
    cxNoRetur.Text:=q2r.FieldByName('no_returbeli').AsString;
...
//    end;
  ...
  try
    qd2r.Active := false;
    qd2r.SQL.Clear;
    qd2r.SQL.Text :='select * from detail_returbeli WHERE no_trans_returbeli ='+
   ...
   // Total;
    ...
    while not qd2r.Eof do begin
          ...
          Grid1.Cells[5,counter]:=dataRow(qd2r.FieldByName('retur'),'0');
