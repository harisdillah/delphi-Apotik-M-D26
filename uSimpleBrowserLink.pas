...
uses
  ...
  Controls, Forms, Dialogs, ExtCtrls, ComCtrls, StdCtrls, uWVBrowserBase,
  uWVBrowser, uWVWinControl, uWVWindowParent;
  {$ENDIF}
...
type
  ...
  public
    { Public declarations }
    procedure TampilBrowser(Link: string);
  end;

var
  FrmBrowserLink: TFrmBrowserLink;
...
procedure TFrmBrowserLink.FormShow(Sender: TObject);
begin
...
procedure TFrmBrowserLink.GoBtnClick(Sender: TObject);
begin
  WVBrowser1.Navigate(AddressCb.Text);
end;

procedure TFrmBrowserLink.WVBrowser1AfterCreated(Sender: TObject);
begin
...
procedure TFrmBrowserLink.WVBrowser1DocumentTitleChanged(Sender: TObject);
begin
  Caption := 'SimpleBrowser - ' + WVBrowser1.DocumentTitle;
end;

procedure TFrmBrowserLink.WVBrowser1InitializationError(Sender: TObject;
  aErrorCode: HRESULT; const aErrorMessage: wvstring);
begin
  showmessage(aErrorMessage);
end;

procedure TFrmBrowserLink.Timer1Timer(Sender: TObject);
begin
...
procedure TFrmBrowserLink.WMMove(var aMessage : TWMMove);
begin
  inherited;
...
procedure TFrmBrowserLink.WMMoving(var aMessage : TMessage);
begin
  inherited;
...
procedure TFrmBrowserLink.TampilBrowser(Link: string);
begin
  AddressCb.Text := Link;
  WVBrowser1.Navigate(AddressCb.Text);
end;
